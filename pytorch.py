...
Original file is located at
    https://colab.research.google.com/drive/1i5dvINPBKkfsA5FIMBLZvElekDVKf8KV
"""
...
FOLDERNAME = 'cs231n_assignments/assignment2'
...
def two_layer_fc(x, params):
    ...
    x = F.relu(x.mm(w1)) # mm == matrix multiply
    x = x.mm(w2)
    return x
...
def three_layer_convnet(x, params):
    ...
    flatten_relu_2=flatten(relu_2)
    fc_1=flatten_relu_2.mm(fc_w)
    scores=fc_1+fc_b
...
def three_layer_convnet_test():
    ...
    # 64 images, each was extracted by their out_channel(==6) characters,torch.Size([64, 6, 32, 32])
    conv_w1 = torch.zeros((6, 3, 5, 5), dtype=dtype)  # [out_channel, in_channel, kernel_H, kernel_W]
    conv_b1 = torch.zeros((6,))  # out_channel
    # 64 images, each was extracted by their out_channel(==9) characters,torch.Size([64, 9, 32, 32])
    conv_w2 = torch.zeros((9, 6, 3, 3), dtype=dtype)  # [out_channel, in_channel, kernel_H, kernel_W]
    conv_b2 = torch.zeros((9,))  # out_channel

    # you must calculate the shape of the tensor after two conv layers, before the fully-connected layer
    fc_w = torch.zeros((9 * 32 * 32, 10))# 64 images, each has 9*32*32 characters, then was classified into 10 classes
    fc_b = torch.zeros(10)
...
def check_accuracy_part2(loader, model_fn, params):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            _, preds = scores.max(1)# values,indices
            num_correct += (preds == y).sum()
            num_samples += preds.size(0)# using size, not shape
...
# you must calculate the shape of the tensor after two conv layers, before the fully-connected layer
fc_w = random_weight((9 * 32 * 32, 10))# 64 images, each has 9*32*32 characters, then was classified into 10 classes
fc_b = zero_weight(10)
...
class ThreeLayerConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        before_fc = F.relu(self.conv2(F.relu(self.conv1(x))))
        before_fc = flatten(before_fc)
        scores = self.fc1(before_fc) # before fc layer we need to flatten the vector
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ########################################################################
        #                             END OF YOUR CODE                         #
        ########################################################################
